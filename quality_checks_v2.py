...
def score_data_models(val_schema_path, val_weights_path, m_path, data_models, debug_out=False):
    ...
    if debug_out:
        ...
        fname = os.path.join(CWD, 'reports', 'Debug and Test v2.xlsx')
...
def write_metadata_quality(dm_scores):
    ...
    fname = os.path.join(CWD, 'reports', 'metadata_quality.v2.json')
    write_timestamp(f"{fname}")
    ...
    fname = os.path.join(CWD, 'reports', 'metadata_quality.v2.csv')
    write_timestamp(f"{fname}")
