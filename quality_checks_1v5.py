...
def main():
    ...
    export_json(completeness_score,'reports/v1.5/completeness.json')
    export_csv(completeness_score, 'reports/v1.5/completeness.csv', headers)

    # Compile Schema Validation Error Score
    schema_errors, headers = schema_validation_check()
    export_json(schema_errors,'reports/v1.5/schema_errors.json')
    export_csv(schema_errors, 'reports/v1.5/schema_errors.csv', headers)
    ...
    export_json(attribute_completeness_score,'reports/v1.5/attribute_completeness.json')
    csv_data, headers = flatten_reporting_dict(attribute_completeness_score)
    export_csv(csv_data, 'reports/v1.5/attribute_completeness.csv', headers)

    # Compile Attribute Schema Validation Error Score
    attribute_schema_errors = check_attribute_validation(data_models)
    export_json(attribute_schema_errors,'reports/v1.5/attribute_errors.json')
    csv_data, headers = flatten_reporting_dict(attribute_schema_errors)
    export_csv(csv_data, 'reports/v1.5/attribute_errors.csv', headers)

    # Summarise Average Quality Score
    summary_score, headers = generate_quality_score()
    export_json(summary_score,'reports/v1.5/metadata_quality.json')
    export_csv(summary_score, 'reports/v1.5/metadata_quality.csv', headers)

    # # Generate Data Utility Framework scores
    utility_scores, headers = read_csv('reports/v1.5/data_utility.csv')
    # utility_scores = update_utility_scores(summary_score, utility_scores, headers)
    utility_scores = update_utility_scores(summary_score, utility_scores, headers)
    export_json(utility_scores,'reports/v1.5/data_utility.json')
    export_csv(utility_scores, 'reports/v1.5/data_utility.csv', headers)
